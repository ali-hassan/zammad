class CommunicateWhatsappJob < ApplicationJob
  ...
  def perform(article_id)
    ...
    log_error(article, "Can't find ticket.preferences['whatsapp']['chat_id'] for Ticket.find(#{article.ticket_id})") if !ticket.preferences['whatsapp']['chat_id']
    if ticket.preferences['whatsapp'] && ticket.preferences['whatsapp']['bid']
      channel = Whatsapp.bot_by_bot_id(ticket.preferences['whatsapp']['bid'])
    end
    if !channel
    ...
    begin
      api = Whats::Api.new(channel.options[:api_token])
      chat_id = ticket.preferences[:whatsapp][:chat_id]
    ...
    if result['from'] && result['chat']
      ...
      # fill article with message info (whatsapp channel)
    ...
    article.message_id = "whatsapp.#{result['message_id']}.#{result['chat']['id']}"
