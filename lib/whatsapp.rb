...
=end
  ...
  def self.create_or_update_channel(token, params, channel = nil)
    ...
    if !channel && Whatsapp.bot_duplicate?(bot['id'])
    ...
    callback_url = "#{Setting.get('http_type')}://#{Setting.get('fqdn')}/api/v1/channels_whatsapp_webhook/#{callback_token}?bid=#{bot['id']}"
    Whatsapp.set_webhook(token, callback_url)

    if !channel
      channel = Whatsapp.bot_by_bot_id(bot['id'])
      if !channel
        channel = Channel.new
      end
    end
...
get channel by bot_id
  ...
  channel = Whatsapp.bot_by_bot_id(bot_id)
...
generate message_id for message
  ...
  message_id = Whatsapp.message_id(message)

returns

  message_id # 123456@whatsapp
...
=end
  ...
  def to_article(params, user, ticket, channel, article = nil)
    ...
    if article
      ...
      article = Ticket::Article.new(
        ...
        message_id:  Whatsapp.message_id(params),
        internal:    false,
        preferences: {
          message:   {
            created_at: params[:message][:date],
  ...
  def to_group(params, group_id, channel)
    ...
    if params[:edited_channel_post]
      # updates on whatsapp can only be on messages, no attachments
    ...
    return if !params[:edited_message] && Ticket::Article.exists?(message_id: Whatsapp.message_id(params))

    # update article
    if params[:edited_message]
      article = Ticket::Article.find_by(message_id: Whatsapp.message_id(params))
      return if !article
  ...
  def get_file(params, file)
    ...
    # whatsapp bot files are limited up to 20MB
    # https://core.whatsapp.org/bots/api#getfile
  ...
  def download_file(file_id)
    ...
    url = "https://api.whatsapp.org/file/bot#{@token}/#{document['file_path']}"
    UserAgent.get(
      url,
      {},
      {
